...
public class BlockBasic extends Block {
	...
	public BlockBasic(Material material, String name, String textureName, SoundType sound){
		...
		this.setBlockTextureName("RoA:" + textureName);
		this.setStepSound(blockSound);
	}

	@Override
	public boolean canSustainPlant(IBlockAccess world, int x, int y, int z, ForgeDirection direction, IPlantable plantable) {
		if(world.getBlock(x, y, z) == RoABlocks.KyroanSoil){
			return true;
		}
		if(world.getBlock(x, y, z) == RoABlocks.AngelicMarble){
			return true;
		}
		else{
			return false;
	...
	public void onPlantGrow(World world, int x, int y, int z, int sourceX, int sourceY, int sourceZ) {
		if (this == RoABlocks.KyroanSoil || this == RoABlocks.AngelicMarble)
		{
			world.setBlock(x, y, z, RoABlocks.KyroanSoil, 0, 2);
		}
	}
	...
	{
		return Item.getItemFromBlock(RoABlocks.KyroanSoil);
	}
