...
import lib.utility.GameLogHelper;
...
public class ChunkProviderForest implements IChunkProvider {
						...
						for (int i3 = 0; i3 < 4; ++i3) {
							...
							for (int k3 = 0; k3 < 4; ++k3) {
								if ((d15 += d16) > 0.0D) {
									/*
									 * ~~~~~~~~~~~~~~~~~~~~~~~~
									 * Set the main blocks here
									 * ~~~~~~~~~~~~~~~~~~~~~~~~
									 */
									blocks[j3 += short1] = RoABlocks.DestinyStone;
								} else if (k2 * 8 + l2 < b0) {
									/*
									 * ~~~~~~~~~~~~~~~~~~~~~~~~~
									 * Set the main liquids here
									 * ~~~~~~~~~~~~~~~~~~~~~~~~~
									 */
									blocks[j3 += short1] = RoABlocks.blockDemonBlood;//these can be set to custom blocks
								} else {
									/*
									 * air
									 */
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		if (biomegenbase != BiomeGenBase.desert && biomegenbase != BiomeGenBase.desertHills && !flag && this.rand.nextInt(4) == 0
			...
			(new WorldGenLakes(RoABlocks.blockDemonBlood)).generate(this.worldObj, this.rand, k1, l1, i2);
		}
